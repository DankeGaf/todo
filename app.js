...
const buttonShowUnCompleted = document.querySelector(".block__form-button-filter-uncompleted")
...
buttonShowUnCompleted.addEventListener("click", (e) => {
    const list = blockList.childNodes; //список элементов списка
    list.forEach(node => {
        if (node.attributes) { // проверяем есть ли атрибуты у элемента
            const isCompleted = node.attributes['data-deleted'].value === '0'; // проверяем зачеркнут ли элемент
            if (!isCompleted) {
                const classList = node.classList; // получаем список классов элемента
                const isHided = classList.contains("block__list-element_hide"); // проверяем спрятан ли он уже
                if (isHided) {
...
function createElements(text){
    ...
    newElement.setAttribute('data-amount', amount)
